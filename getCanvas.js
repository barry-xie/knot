...
function createCanvasClient(token, apiBase = DEFAULT_API_BASE) {
  ...
  async function getSyllabus(courseId) {
    try {
      const course = await canvasGet(`/courses/${courseId}`, { 'include[]': 'syllabus_body' });
  ...
  async function fetchCanvasData() {
    ...
    for (const course of activeCourses) {
      const [assignments, files, syllabus] = await Promise.all([
      ...
      if (courseData.assignments.length > 0 || courseData.files.length > 0) {
        out.push(courseData);
      }
    }
